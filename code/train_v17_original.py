# -*- coding: utf-8 -*-
"""
OFC 2026 ML Challenge - VERSION 17.0 (Ultimate Finisher)
Winning Strategy:
1. Distilled Multi-Teacher Pseudo-Labeling (0.089 + 0.094 teacher).
2. Transformer-UNet + FGM Adversarial Training.
3. Snapshot Ensembling: Save 3 best checkpoints per fold.
4. Dense TTA: Multidimensional grid search perturbation during inference.
5. Physics-Guided Template Refinement.
"""
...
OUT_SUB_CSV        = "../Features/Test/submission_v17_ultimate.csv"
...
class TransformerUNet(nn.Module):
    def __init__(self, nch, global_dim, n_dev):
        ...
        self.bottleneck_attn = nn.MultiheadAttention(256, 8, batch_first=True) # More heads
    ...
    def forward(self, load, pow, cfg, dev_id):
        # Precise Quadratic Physics Template
...
def prep():
    print("V17.0 Ultimate Preprocessing...")
    ...
    xc_tr = _fin(cr_tr, ct_tr, st_tr, co_tr, tr_f["EDFA_type"])
    xc_te = _fin(cr_te, ct_te, st_te, co_te, te_f["EDFA_type"])
    xc_cs = _fin(cr_cs, ct_cs, st_cs, co_cs, cs_f["EDFA_type"])
...
def main():
    ...
    fgm = FGM(model); kf = KFold(N_FOLDS, shuffle=True, random_state=BASE_SEED)
    ...
    for f, (ti, vi) in enumerate(kf.split(l_pool), 1):
        ...
        # Inference for current fold using snapshots + Dense TTA
        fold_pred = np.zeros_like(pred_sum)
        ...
        for snap in snapshots:
            ...
            with torch.no_grad():
                for i in range(0, len(lp), 512):
                    p_base = model(lp[i:i+512], pp[i:i+512], cp[i:i+512], dp[i:i+512])
                    # Multi-dim TTA
                    p_tta1 = model(lp[i:i+512], pp[i:i+512], cp[i:i+512] + torch.cat([torch.tensor([0.25, 0], device=DEVICE), torch.zeros(12, device=DEVICE)]), dp[i:i+512])
                    p_tta2 = model(lp[i:i+512], pp[i:i+512], cp[i:i+512] + torch.cat([torch.tensor([-0.25, 0], device=DEVICE), torch.zeros(12, device=DEVICE)]), dp[i:i+512])
                    p_tta3 = model(lp[i:i+512], pp[i:i+512], cp[i:i+512] + torch.cat([torch.tensor([0, 0.15], device=DEVICE), torch.zeros(12, device=DEVICE)]), dp[i:i+512])
                    p_tta4 = model(lp[i:i+512], pp[i:i+512], cp[i:i+512] + torch.cat([torch.tensor([0, -0.15], device=DEVICE), torch.zeros(12, device=DEVICE)]), dp[i:i+512])
                    fold_pred[i:i+512] += (p_base*0.5 + (p_tta1+p_tta2+p_tta3+p_tta4)*0.125).cpu().numpy()
        pred_sum += fold_pred / len(snapshots)

    res = np.clip(pred_sum / N_FOLDS, -15, 25); res[xl_te == 0] = -1
    ...
    msg = "V17.0: Snapshot Ensemble + Dense Multi-dim TTA + Distilled Pseudo + Adv-Transformer"
    os.system(f"KAGGLE_CONFIG_DIR='/home/ubuntu/Desktop/ofc-competition' /home/ubuntu/Desktop/ofc-competition/.mmenv/bin/kaggle competitions submit -c ofc-2026-ml-challenge -f {OUT_SUB_CSV} -m '{msg}'")
